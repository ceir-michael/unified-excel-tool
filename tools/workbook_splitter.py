...
from core.excel_helpers import copy_cell, worksheet_headers
from core.file_utils import check_cancelled, safe_file_name, safe_sheet_name


def run_workbook_splitter(params: dict, log: Callable[[str], None], cancel: Callable[[], bool] | None = None) -> str:
    ...
    try:
        ...
        headers = worksheet_headers(source, header_row)
        ...
        for row_number in range(header_row + 1, source.max_row + 1):
            ...
            tab_value = source.cell(row=row_number, column=tab_index).value if tab_index else None
            grouped.setdefault(split_value, {}).setdefault(tab_value, []).append(row_number)
    except BaseException:
        workbook.close()
        raise

    staging = Path(tempfile.mkdtemp(prefix=".unified-excel-", dir=output_folder))
    staged_files: list[tuple[Path, Path]] = []
    published: list[Path] = []
    used_file_names = {path.name.casefold() for path in output_folder.iterdir() if path.is_file()}
    try:
        for split_value, tab_groups in grouped.items():
            ...
            for tab_value, source_rows in tab_groups.items():
                ...
                log(f"{split_value}: writing tab '{sheet_name}' ({len(source_rows)} row(s))")
                ...
                for target_row, source_row in enumerate(source_rows, start=2):
                    check_cancelled(cancel)
                    for column_number in range(1, source.max_column + 1):
                        copy_cell(source.cell(row=source_row, column=column_number), target.cell(row=target_row, column=column_number))
                target.auto_filter.ref = target.dimensions

            base_name = f"{input_path.stem} - {safe_file_name(split_value)}"
            file_name = f"{base_name}.xlsx"
            counter = 1
            while file_name.casefold() in used_file_names:
                file_name = f"{base_name}_{counter}.xlsx"
                counter += 1
            used_file_names.add(file_name.casefold())
            staged_path = staging / file_name
            output_workbook.save(staged_path)
            output_workbook.close()
            staged_files.append((staged_path, output_folder / file_name))

        check_cancelled(cancel)
        for staged_path, output_path in staged_files:
            if output_path.exists():
                raise FileExistsError(f"Output appeared while processing: {output_path.name}")
            os.replace(staged_path, output_path)
            published.append(output_path)
            log(f"Saved {output_path.name}")
        return f"Created {len(published)} workbook(s) in {output_folder}"
    except BaseException:
